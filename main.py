...
import pandas as pd
import os, tarfile, tempfile, requests

def get_imdb_dataset():
    # download raw IMDB tarball from Stanford
    url = "https://ai.stanford.edu/~amaas/data/sentiment/aclImdb_v1.tar.gz"
    tmpdir = tempfile.mkdtemp()
    tar_path = os.path.join(tmpdir, "aclImdb_v1.tar.gz")
    print("Downloading IMDB raw dataset (≈80MB)...")
    ...
    def read_split(split):
        ...
        for label in ["pos", "neg"]:
            dir_path = os.path.join(tmpdir, f"aclImdb/{split}/{label}")
...
def create_augmented_dataloader(args, dataset):
    ...
    # 1️⃣ randomly select 5 000 samples from the training data
    aug_samples = dataset["train"].shuffle(seed=42).select(range(5000))

    # 2️⃣ apply your Q2 transformation to these samples
    aug_transformed = aug_samples.map(custom_transform, load_from_cache_file=False)

    # 3️⃣ combine original + transformed datasets
    augmented_train = datasets.concatenate_datasets([dataset["train"], aug_transformed])

    # 4️⃣ tokenize the combined dataset
    tokenized_aug = augmented_train.map(tokenize_function, batched=True, load_from_cache_file=False)
